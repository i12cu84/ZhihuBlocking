...
解决了第一个栏目包含屏蔽词时整个页面内容都被屏蔽的问题

现在每个问题都会独立检查屏蔽词，互不影响

-> 修复重新扫描功能

改进rescanAllContent()函数，清除缓存时同时移除样式标记

确保重新扫描时所有内容都能被正确处理

-> 增强空值检查

在shouldBlock()和processQuestionElement()函数中添加空值检查

避免处理空标题时出现错误

-> 优化元素选择器

使用更精确的选择器匹配知乎问题容器和标题

减少误匹配和漏匹配的情况

-> 改进MutationObserver处理逻辑
